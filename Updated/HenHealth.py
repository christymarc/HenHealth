...
class Create(Screen):
    ...
    def submit(self):
        ...
        if (phoneNum != "") and (username != "") and (password != ""):
            if (username not in df_main['username']):
                if (phoneNum not in df_main['phonenumber']) and (len(phoneNum) == 10) and phoneNum.isdigit(): 
...
def show_covid_message(self, button):
    """
    ...
    closeButton = Button(text = "X", anchor_x = "right", anchor_y = "top") 
